...
- **Widgets are for UI:** Everything in Flutter's UI is a widget. Compose
  complex UIs from smaller, reusable widgets.You cannot return a widget using a method.
  Generic components should be encapsulated in a new file created in the widgets folder, and comments should be written indicating what they are applicable to.
  To ensure performance, unnecessary rendering should be minimized in the interface.
...
- **Text:** Use `Theme.of(context).textTheme` for text styles.
  remote images.
    ...
    The colors used in a widget should be the theme colors, which can be obtained as follows:
  ```dart
