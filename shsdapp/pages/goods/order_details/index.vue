<template>
  <view :style="colorStyle">
    <view class='order-details'>
      ...
      <view class="position-relative flex align-center justify-center">
        <uni-icons class="position-absolute left-0 ml-3" type="arrow-left" size="30" color="#FFFFFF" @click="navigateBack"></uni-icons>
        <text style="color: #FFFFFF;font-size: 32rpx;font-weight: 600;">确认订单</text>
      </view>
      <!-- 给header上与data上加on为退款订单-->
      <view class='header  acea-row flex flex-column' :class='isGoodsReturn ? "on":""'>
<!--        <view class='pictrue' v-if="isGoodsReturn==false">-->
<!--          <image :src="orderInfo.status_pic"></image>-->
<!--        </view>-->
<!--        待付款-->
        <view class='stay-payment' v-if="isGoodsReturn==false">
...
<!--        </view>-->
        ...
        <view v-if="orderInfo.virtual_type == 0">
          ...
          <view class='address' v-else>
            <view class='name' @tap="makePhone">{{ orderInfo.system_store.name }}
              <text
                  class='phone'>{{ orderInfo.system_store.phone }}
              </text>
              <text
                  class="iconfont icon-tonghua font-num"></text>
            </view>
...
<!--      </div>-->
      ...
      <view class='wrapper' v-if="orderInfo.total_price">
        <view class='item acea-row row-between'>
          <view class="shop-total-price-text">{{ $t(`商品总价`) }}</view>
          <view class='conter shop-total-price'>
            {{ $t(`消费券`) }}{{ (parseFloat(orderInfo.total_price) + parseFloat(orderInfo.vip_true_price)).toFixed(2) }}
          </view>
        </view>
        <view class='item acea-row row-between'>
          <view class="shop-total-price-text">{{ $t(`运费`) }}</view>
          <view class='conter' style="font-weight: 500;font-size: 24rpx;color: #000000;">
           免运费
          </view>
        </view>
        <view class='item acea-row row-between' v-if="orderInfo.pay_postage > 0">
          <view>{{ $t(`配送运费`) }}：</view>
          <view class='conter'>{{ $t(`消费券`) }}{{ parseFloat(orderInfo.pay_postage).toFixed(2) }}</view>
        </view>
        <view v-if="orderInfo.levelPrice > 0" class='item acea-row row-between'>
          <view>{{ $t(`用户等级优惠`) }}：</view>
          <view class='conter'>-{{ $t(`消费券`) }}{{ parseFloat(orderInfo.levelPrice).toFixed(2) }}</view>
        </view>
        <view v-if="orderInfo.memberPrice > 0" class='item acea-row row-between'>
          <view>{{ $t(`付费会员优惠`) }}：</view>
          <view class='conter'>-{{ $t(`消费券`) }}{{ parseFloat(orderInfo.memberPrice).toFixed(2) }}</view>
        </view>
        <view class='item acea-row row-between' v-if='orderInfo.coupon_price > 0'>
          <view>{{ $t(`消费券抵扣`) }}：</view>
          <view class='conter'>-{{ $t(`消费券`) }}{{ parseFloat(orderInfo.coupon_price).toFixed(2) }}</view>
        </view>
        <view class='item acea-row row-between' v-if="orderInfo.use_integral > 0">
          <view>{{ $t(`消费券抵扣`) }}：</view>
          <view class='conter'>-{{ $t(`消费券`) }}{{ parseFloat(orderInfo.deduction_price).toFixed(2) }}</view>
        </view>
        <view class='actualPay acea-row justify-between shifukuan-text' v-if="!orderInfo.help_info.help_status">{{ $t(`商品总价`) }}
          <text
              class='money shifukuan-price'>{{ $t(`消费券`) }}{{ parseFloat(orderInfo.pay_price).toFixed(2) }}
          </text>
        </view>
        <view class='actualPay acea-row row-right' v-else>
          ...
          {{ $t(`总代付`) }}：
          <text
              class='money font-color'>{{ $t(`消费券`) }}{{ parseFloat(orderInfo.pay_price).toFixed(2) }}
          </text>
        </view>
      </view>
      <view class='wrapper'>
        <view class="order-card">订单信息</view>
        <view class='item acea-row row-between'>
          <view>{{ $t(`订单号`) }}：</view>
          <view class='conter acea-row row-middle row-right'>
            <text class="orderInfo-font">{{ orderInfo.order_id }}</text>
            <!-- #ifndef H5 -->
...
<!--            <text class='copy copy-data' :data-clipboard-text="orderInfo.order_id">{{ $t(`复制`) }}</text>-->
        ...
        <view class='item acea-row row-between'>
          <view>{{ $t(`下单时间`) }}：</view>
          <view class='conter orderInfo-font'>{{ (orderInfo.add_time_y || '') + ' ' + (orderInfo.add_time_h || 0) }}</view>
        </view>
        <view class='item acea-row row-between'>
          <view>{{ $t(`支付状态`) }}：</view>
          <view class='conter orderInfo-font' v-if="orderInfo.paid">{{ $t(`已支付`) }}</view>
          <view class='conter orderInfo-font' v-else>{{ $t(`未支付`) }}</view>
        </view>
        <view class='item acea-row row-between' v-if="orderInfo.paid">
          <view>{{ $t(`支付方式`) }}：</view>
          <view class='conter orderInfo-font'>{{ $t(orderInfo._status._payType) }}</view>
        </view>
        <view class='item acea-row row-between' v-if="orderInfo.mark && isReturen != 1">
          ...
          <view class='conter orderInfo-font'>{{ orderInfo.mark }}</view>
        </view>
        <view class='item acea-row row-between' v-if="orderInfo.remark">
          <view>{{ $t(`商家备注`) }}：</view>
          <view class='conter orderInfo-font'>{{ orderInfo.remark }}
            <!-- #ifndef H5 -->
...
<!--              @click="more">{{ $t(`更多`) }}<span class='iconfont icon-xiangshang'></span></view>-->
        <view  v-if="(invoice_func || invoiceData) && orderInfo.paid && !orderInfo.refund_status"></view>
...
<!--        </view>-->
    ...
    <invoice-picker :inv-show="invShow" :is-special="special_invoice" :url-query="urlQuery"
                    :inv-checked="invChecked" :order-id='order_id' :inv-list="invList" :is-order="1"
                    @inv-close="invClose"
