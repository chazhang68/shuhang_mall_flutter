...
<!--				</view>-->
				<view class='address acea-row row-between-wrapper' @tap='onAddress' v-if='shippingType == 0'>
					<view class='addressCon' v-if="addressInfo.real_name || ''">
						...
						<view class="line1" style="margin-top: 25rpx">
...
<!--					</view>-->
<!--				</view>-->
<!--&lt;!&ndash; 				<view class='item acea-row row-between-wrapper'-->
<!--					v-if="!pinkId && !BargainId && !combinationId && !seckillId && !advanceId && integral_open">-->
<!--					<view>{{$t(`消费券抵扣`)}}</view>-->
<!--					<view class='discount acea-row row-middle'>-->
<!--						<view> {{useIntegral ? $t(`剩余消费券`):$t(`当前消费券`)}}-->
...
<!--			</view>-->
			...
			<view class='moneyList'>
				<view class='item acea-row row-between-wrapper'>
					...
					<view class='money shop-total-price'>
						{{$t(`消费券`)}}{{allPrice || 0}}
					</view>
				</view>
        <view class='item acea-row row-between-wrapper'>
					<view class="shop-total-price-text">{{$t(`运费`)}}</view>
					<view class='money' style="font-weight: 500;font-size: 24rpx;color: #000000;">
						免运费
					</view>
				</view>
        <view class='actualPay acea-row justify-between shifukuan-text'>{{ $t(`商品总价`) }}
          <text class='money shifukuan-price'>
            {{$t(`消费券`)}}{{allPrice || 0}}
          </text>
        </view>
				<view class='item acea-row row-between-wrapper'
					...
					<view>{{$t(`配送运费`)}}：</view>
					<view class='money'>
						{{$t(`消费券`)}}{{(parseFloat(priceGroup.storePostage)+parseFloat(priceGroup.storePostageDiscount)).toFixed(2)}}
					</view>
				</view>
				<view class='item acea-row row-between-wrapper'
					v-if="priceGroup.levelPrice > 0 && userInfo.vip && !pinkId && !BargainId && !combinationId && !seckillId && !discountId">
					<view>{{$t(`用户等级优惠`)}}：</view>
					<view class='money'>-{{$t(`消费券`)}}{{parseFloat(priceGroup.levelPrice).toFixed(2)}}</view>
				</view>
				<view class='item acea-row row-between-wrapper'
					v-if="priceGroup.memberPrice > 0 && userInfo.vip && !pinkId && !BargainId && !combinationId && !seckillId && !discountId">
					<view>{{$t(`付费会员优惠`)}}：</view>
					<view class='money'>-{{$t(`消费券`)}}{{parseFloat(priceGroup.memberPrice).toFixed(2)}}</view>
				</view>
				<view class='item acea-row row-between-wrapper' v-if="priceGroup.storePostageDiscount > 0">
					<view>{{$t(`会员运费优惠`)}}：</view>
					<view class='money'>-{{$t(`消费券`)}}{{parseFloat(priceGroup.storePostageDiscount).toFixed(2)}}</view>
				</view>
				<view class='item acea-row row-between-wrapper' v-if="coupon_price > 0">
					<view>{{$t(`消费券抵扣`)}}：</view>
					<view class='money'>-{{$t(`消费券`)}}{{parseFloat(coupon_price).toFixed(2)}}</view>
				</view>
				<view class='item acea-row row-between-wrapper' v-if="integral_price > 0">
					<view>{{$t(`消费券抵扣`)}}：</view>
					<view class='money'>-{{$t(`消费券`)}}{{parseFloat(integral_price).toFixed(2)}}</view>
				</view>
			</view>
			<view style='height:120rpx;'></view>
			<view class='footer acea-row row-between-wrapper'>
				<view>{{$t(`合计`)}}:
					<text style="font-family: DIN Alternate;font-weight: bold;font-size: 36rpx;color: #FF5A5A;">{{$t(`消费券`)}}{{totalPrice || 0}}</text>
				</view>
	...
	export default {
		...
		data() {
			...
			return {
				...
				userInfo: {}, //用户信息
				mark: '', //备注信息
				couponTitle: this.$t(`请选择`), //消费券
				coupon_price: 0, //消费券抵扣金额
				useIntegral: false, //是否使用消费券
				integral_price: 0, //消费券抵扣金额
				integral: 0,
				usable_integral: 0,
				ChangePrice: 0, //使用消费券抵扣变动后的金额
				formIds: [], //收集formid
				status: 0,
				is_address: false,
