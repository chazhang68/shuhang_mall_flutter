	...
	<view :style="colorStyle">
		<view class="my-order">
			...
			<view class="nav acea-row row-around">
				<view class="item flex flex-column align-center" @click="statusClick(9)">
					<view>{{$t(`全部`)}}</view>
          <view class="item-active" v-if="orderStatus == 9"></view>
				</view>
				<view class="item flex flex-column align-center" @click="statusClick(0)">
					<view>{{$t(`待付款`)}}</view>
          <view class="item-active" v-if="orderStatus == 0"></view>
				</view>
				<view class="item flex flex-column align-center" @click="statusClick(1)">
					<view>{{$t(`待发货`)}}</view>
          <view class="item-active" v-if="orderStatus == 1"></view>
				</view>
				<view class="item flex flex-column align-center" @click="statusClick(2)">
					<view>{{$t(`待收货`)}}</view>
          <view class="item-active" v-if="orderStatus == 2"></view>
				</view>
				<view class="item flex flex-column align-center" @click="statusClick(3)">
					<view>{{$t(`已完成`)}}</view>
          <view class="item-active" v-if="orderStatus == 3"></view>
				</view>
...
<!--								<view>{{ item._add_time }}</view>-->
                <view class="flex align-center">
                  <text class="card-id line1">订单编号：{{item.order_id}}</text>
<!--                  <text class="copy">复制</text>-->
                ...
                <view>
                  ...
                  <text class="line2"></text>
								...
								<view class="money">
									<view style="color: black" v-if="items.productInfo.attrInfo">
										{{$t(`消费券`)}}{{ items.productInfo.attrInfo.price }}
									</view>
									<view style="color: black" v-else>{{$t(`消费券`)}}{{ items.productInfo.price }}</view>
						...
						<view class="totalPrice">
							<text style="color: #9999a6;padding-right: 18rpx">{{$t(`共`)}}{{ item.total_num || 0 }}{{$t(`件商品`)}}</text>
              {{$t(`合计`)}}
							<text class="money">{{$t(`消费券`)}}{{ item.pay_price }}</text>
						</view>
					</view>
            ...
            <view class="bnt payBnt" v-if="item.delivery_type == 'express' && item._status._type==2 " @click="gowuliu(item.order_id)">{{$t(`查看物流`)}}</view>
				<view class="bnt bg-color" style="border: unset;color: #fff;" @click="goOrderDetails(item.order_id)">{{$t(`查看详情`)}}</view>
     <!--       <view class="bnt payBnt" v-if="item._status._type == 1 && item.shipping_type == 1">{{$t(`催发货`)}}</view -->

						<view class="bnt payBnt" v-if="item._status._type == 0"
							@click="goPay(item.pay_price, item.order_id)">{{$t(`去支付`)}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			gowuliu(orderId){
				uni.navigateTo({
					url:'/pages/goods/goods_logistics/index?orderId='+orderId,
