<template>
	<view class="page" v-if="payPriceShow">
				...
				<view class="left acea-row row-between-wrapper">
					...
					<view class="text">
						...
						<view class="info" v-if="item.value == 'yue'">
							{{$t(item.title)}} <span class="money">{{$t(`消费券`)}}{{ item.number }}</span>
						</view>
						<view class="info" v-else-if="item.value == 'xfq'">
							{{$t(item.title)}} <span class="money">{{ item.number }}</span>
						</view>
		...
		<view class="btn">
			<view class="button acea-row row-center-wrapper" @click='goPay(number, paytype,integral)'>{{$t(`确认支付`)}}</view>
			<view class="wait-pay" @click="waitPay">{{$t(`暂不支付`)}}</view>
		</view>
		<view v-show="false" v-html="formContent"></view>
		<view>
			<!-- 输入框示例 -->
			<uni-popup ref="code" type="dialog">
				<uni-popup-dialog ref="inputClose"  mode="input" title="支付确认验证码" value=""
					placeholder="请输入支付确认验证码" @confirm="dialogInputConfirm"></uni-popup-dialog>
			</uni-popup>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			dialogInputConfirm(code){//银行确认
				let that = this;
				uni.showLoading({
					title:'支付确认中'
				})
				bankOkPay({code:code,order_id:that.orderId}).then(res=>{
					uni.hideLoading()
			...
			getCashierOrder() {
				...
				getCashierOrder(this.orderId, this.fromType).then(res => {
					...
					this.cartArr[5].number = res.data.integral;
					if(res.data.order_type == 1){
						this.cartArr[5].payStatus = 1 
					}else{
						this.cartArr[5].payStatus = 0 
					}
					this.number = Number(res.data.now_money) || 0;
					this.integral = Number(res.data.integral) || 0;
					uni.hideLoading();
				}).catch(err => {
					uni.hideLoading();
					return this.$util.Tips({
...
<style lang="scss" scoped>
	...
	.page {
		...
		.payment .item .left .iconfont.icon-yinhangqia {
			color:red;
		}
