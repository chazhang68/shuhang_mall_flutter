<template>
	<view class="login-wrapper" :style="colorStyle">
		<view class="shading">
			<image :src="logoUrl" style="border-radius: 100%;" />
		</view>
				...
				<form @submit.prevent="submit">
					<view class="item">
						<view class="acea-row row-middle">
							...
							<input type="text" :placeholder="$t(`输入账号`)" v-model="account" maxlength="11" required />
						</view>
					</view>
					<view class="item">
						<view class="acea-row row-middle">
							<image class="position-relative" src="../static/code_1.png" style="width: 28rpx; height: 32rpx;z-index: 99"></image>
              <!-- #ifdef APP-PLUS -->
              <uni-easyinput style="margin-left: -11px" :placeholderStyle="inputStyle" :clearable="false" type="password" :input-border="false" v-model="password" :placeholder="$t(`填写登录密码`)"></uni-easyinput>
              <!-- #endif -->
              <!-- #ifndef APP-PLUS -->
              <uni-easyinput :placeholderStyle="inputStyle" :clearable="false" type="password" :input-border="false" v-model="password" :placeholder="$t(`填写登录密码`)"></uni-easyinput>
              <!-- #endif -->
<!--							<input type="password" :placeholder="$t(`填写登录密码`)" v-model="password" required />-->
						</view>
					</view>
				</form>
				<navigator style="padding-top: 10px;padding-left: 17px;" class="forgetPwd" hover-class="none" url="/pages/users/retrievePassword/index">
			...
			<view class="list" v-if="current == 1 || appLoginStatus || appleLoginStatus">
				<view class="item">
					<view class="acea-row row-middle">
						<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx;"></image>
						<input type="text" :placeholder="$t(`输入账号`)" v-model="account" maxlength="11" />
					</view>
				</view>
				<view class="item">
					<view class="acea-row row-middle">
			...
			<view class="list" v-if="current == 2">
				<view class="item">
					<view class="acea-row row-middle">
						<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx;"></image>
						<input type="text" placeholder="输入账号" v-model="account" maxlength="11" />
					</view>
				</view>
				<!-- <view class="item">
					<view class="acea-row row-middle">
				...
				<view class="item">
					<view class="acea-row row-middle">
						<image src="../static/code_1.png" class="position-relative" style="width: 24rpx; height: 34rpx;z-index: 99"></image>
            <!-- #ifdef APP-PLUS -->
            <uni-easyinput style="margin-left: -11px" maxlength="11" :placeholderStyle="inputStyle" :clearable="false" type="password" :input-border="false" v-model="password" :placeholder="$t(`填写登陆密码`)"></uni-easyinput>
            <!-- #endif -->
          
			<!-- #ifndef APP-PLUS -->
            <uni-easyinput maxlength="11" :placeholderStyle="inputStyle" :clearable="false" type="password" :input-border="false" v-model="password" :placeholder="$t(`填写登陆密码`)"></uni-easyinput>
            <!-- #endif -->
<!--						<input type="password" placeholder="填写登陆密码" v-model="password" maxlength="11" />-->
					</view>
				</view>
				<view class="item">
					<view class="acea-row row-middle">
						<image src="../static/phone_1.png" class="position-relative" style="z-index:99;width: 24rpx; height: 34rpx;"></image>
            <!-- #ifdef APP-PLUS -->
            <uni-easyinput style="margin-left: -11px" :placeholderStyle="inputStyle" maxlength="6" :clearable="false" type="password" :input-border="false" v-model="pay_pwd" :placeholder="$t(`填写6位数字支付密码`)"></uni-easyinput>
            <!-- #endif -->
            <!-- #ifndef APP-PLUS -->
            <uni-easyinput :placeholderStyle="inputStyle" maxlength="6" :clearable="false" type="password" :input-border="false" v-model="pay_pwd" :placeholder="$t(`填写6位数字支付密码`)"></uni-easyinput>
            <!-- #endif -->

<!--						<input type="number" placeholder="填写6位数字支付密码" v-model="pay_pwd" maxlength="6" />-->
					</view>
				</view>
				...
				<view class="item">
					<view class="acea-row row-middle">
						<image src="../static/phone_1.png" style="width: 24rpx; height: 34rpx;"></image>
						<input type="text" placeholder="填写邀请码" v-model="spread" maxlength="8" />
					</view>
				</view>
			</view>
			<view class="logon" @click="loginMobile" v-if="current == 1">{{$t(`登录`)}}</view>
			<view class="logon" @click="submit" v-if="current === 0">{{$t(`登录`)}}</view>
			<view class="logon" @click="register" v-if="current === 2">{{$t(`注册`)}}</view>

			<!-- <view class="tips" style="display:flex;align-items:center;justify-content: center;">
				<view v-if="current==0" @click="current = 1">快速登录</view>
				<view v-if="current==1" @click="current = 0">账号登录</view>
				<view v-if="current==2" >已有账号</view>
				<text style="margin: 0px 10px;">|</text>
				<view v-if="current !== 2" @click="current = 2" style="color: #fc1c1c;">注册</view>
				<view v-if="current == 2" @click="current = 1" style="color: #fc1c1c;">去登陆</view>
			</view> -->

			<view class="tips" style="display:flex;align-items:center;justify-content: center;">
				<view v-if="current==0"  style="color: #fc1c1c;">快速登录</view>
				<view v-if="current==1" @click="current = 0" style="color: #fc1c1c;">账号登录</view>
				<view v-if="current==2" >已有账号</view>
				<text style="margin: 0px 10px;">|</text>
				<view v-if="current !== 2" @click="current = 2" style="color: #fc1c1c;">注册</view>
				<view v-if="current == 2" @click="current = 0" style="color: #fc1c1c;">去登陆</view>
			</view>
