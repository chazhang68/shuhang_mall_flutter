...
/// 绿色积分页面
/// 对应原 pages/users/fudou/fudou.vue
class FudouPage extends StatefulWidget {
  const FudouPage({super.key});

  @override
  State<FudouPage> createState() => _FudouPageState();
}

class _FudouPageState extends State<FudouPage> {
  ...
  Future<void> _loadUserInfo() async {
    try {
      ...
      if (response.isSuccess && response.data != null) {
        ...
        setState(() {
          _fudouBalance = (data['fudou'] ?? 0).toString();
        });
      }
    } catch (e) {
  ...
  Future<void> _loadRecordList({bool isRefresh = false}) async {
    ...
    try {
      final response = await _userProvider.getFudouList({
        'page': _page,
        'limit': _limit,
        'pm': _currentType,
      });
      ...
      child: Row(
        ...
        children: [
          Column(
            ...
            children: [
              const Text('我的绿色积分', style: TextStyle(color: Colors.white70, fontSize: 14)),
              const SizedBox(height: 8),
              Text(
                _fudouBalance,
                style: const TextStyle(
                  color: Color(0xFFEDB451),
                  fontSize: 32,
