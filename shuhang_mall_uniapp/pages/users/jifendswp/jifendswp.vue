	...
	<view class="m-2">
		<view class="bg-white rounded p-3">
			...
			<view class="mt-2 flex align-center justify-between py-1">
				<view class="font">可用积分数量{{userInfo.fd_ky}}个</view>
		...
		<view class="mt-2 bg-white flex align-center justify-between font p-3 rounded">
			<view>实际到账数量</view>
			<view style="color: #FF5A5A;">{{dznum}}个</view>
		</view>
	...
	export default {
		...
		methods: {
			xfqZhuan(){
				...
				if(Number(that.jfnum) > Number(that.userInfo.fd_ky)){
					return uni.showToast({
						title:"可用积分不足",
						icon:'none'
					})
				...
				xfqDui({number:that.jfnum}).then(res=>{
					uni.hideLoading()
					uni.showToast({
						title:'兑换成功'		
					})
			...
			allJIfen(){
				let jifen = Math.floor(this.userInfo.fd_ky);
				this.jfnum = jifen
			},
			getUserInfo(){
