      ...
      <view class='wrapper' v-if="orderInfo.total_price">
        <view class='item acea-row row-between'>
          ...
          <view class='conter shop-total-price'>
            {{ $t(`swp`) }}{{ (parseFloat(orderInfo.total_price) + parseFloat(orderInfo.vip_true_price)).toFixed(2) }}
          </view>
        </view>
        <view class='item acea-row row-between'>
        ...
        <view class='item acea-row row-between' v-if="orderInfo.pay_postage > 0">
          <view>{{ $t(`配送运费`) }}：</view>
          <view class='conter'>{{ $t(`swp`) }}{{ parseFloat(orderInfo.pay_postage).toFixed(2) }}</view>
        </view>
        <view v-if="orderInfo.levelPrice > 0" class='item acea-row row-between'>
          <view>{{ $t(`用户等级优惠`) }}：</view>
          <view class='conter'>-{{ $t(`swp`) }}{{ parseFloat(orderInfo.levelPrice).toFixed(2) }}</view>
        </view>
        <view v-if="orderInfo.memberPrice > 0" class='item acea-row row-between'>
          <view>{{ $t(`付费会员优惠`) }}：</view>
          <view class='conter'>-{{ $t(`swp`) }}{{ parseFloat(orderInfo.memberPrice).toFixed(2) }}</view>
        </view>
        <view class='item acea-row row-between' v-if='orderInfo.coupon_price > 0'>
          <view>{{ $t(`优惠券抵扣`) }}：</view>
          <view class='conter'>-{{ $t(`swp`) }}{{ parseFloat(orderInfo.coupon_price).toFixed(2) }}</view>
        </view>
        <view class='item acea-row row-between' v-if="orderInfo.use_integral > 0">
          <view>{{ $t(`消费券抵扣`) }}：</view>
          <view class='conter'>-{{ $t(`swp`) }}{{ parseFloat(orderInfo.deduction_price).toFixed(2) }}</view>
        </view>
        <view class='actualPay acea-row justify-between shifukuan-text' v-if="!orderInfo.help_info.help_status">{{ $t(`商品总价`) }}
          <text
              class='money shifukuan-price'>{{ $t(`swp`) }}{{ parseFloat(orderInfo.pay_price).toFixed(2) }}
          </text>
        </view>
        <view class='actualPay acea-row row-right' v-else>
          ...
          {{ $t(`总代付`) }}：
          <text
              class='money font-color'>{{ $t(`swp`) }}{{ parseFloat(orderInfo.pay_price).toFixed(2) }}
          </text>
        </view>
      </view>
      <view class='wrapper'>
